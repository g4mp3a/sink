...
cluster = Cluster('couchbase://cb-example-0000.cb-example.gautam-mart.svc')
...
def submit_order():
    ...
    key = "Order::{}::{}".format(name, datetime.datetime.utcnow().isoformat())
    data = {
        'type':'order',
        'name':name,
        'ts':int(time.time()),
...
def filter_items():
    ...
    result = bucket.n1ql_query("select meta().id from couchmart where category = '{}'".format(filter_type))
    for row in result:
        keys.append(row['id'])

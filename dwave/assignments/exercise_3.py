...
def define_variables(stockcodes):
    ...
    stocks = [Binary('s_{stk}') for stk in stockcodes]
...
def define_cqm(stocks, num_stocks_to_buy, price, returns, budget, variance):
    ...
    # cqm.add_constraint(quicksum(stocks) == num_stocks_to_buy, label='choose k stocks')
    cqm.add_constraint(quicksum(stocks[i] for i in range(len(stocks))) == num_stocks_to_buy, label='choose k stocks')    
    ...
    cqm.add_constraint(quicksum(price[i]*stocks[i] for i in range(len(stocks))) <= budget, label='budget_limitation')
    ...
    # Maximize returns
    maximize_returns = quicksum(-1*returns[i]*stocks[i] for i in range(len(stocks)))

    # Minimize variance
    minimize_variance = 0
    for i in range(len(stocks)):
        for j in range(i+1, len(stocks)):
            minimize_variance += variance[i][j]*stocks[i]*stocks[j]
    
    # minimize_variance = quicksum(variance[i][j]*stocks[i]*stocks[j] for i in range(len(stocks)) for j in range(i+1, len(stocks)))
    
    cqm.set_objective(maximize_returns + minimize_variance)

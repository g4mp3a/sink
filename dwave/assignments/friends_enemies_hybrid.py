...
def get_bqm(G):
    ...
    # Build the BQM from QUBO
    
    """
    Q = defaultdict(int)
    
    choices = ['friend', 'enemy']

    for i, j in G.edges:
    
        # Randomly assign a relation to this edge
        relationship = random.choice(choices)
    ...
    # Build the BQM using add_variable and add_interaction functions
    bqm = BinaryQuadraticModel('BINARY')
    
    for i, j in G.edges:
        
        # Randomly assign a relation to this edge
        fe = random.choice([1, -1])
        
        bqm.add_variable(i, 1*fe)
        bqm.add_variable(j, 1*fe)
        bqm.add_interaction(i, j, 2*fe)

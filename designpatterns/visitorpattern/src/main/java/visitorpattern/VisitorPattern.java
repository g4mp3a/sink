...
class ProductOfRedNodesVisitor extends TreeVis {
  ...
  private long prod=1;
  private final int c=1000000007;
  ...
  public void visitNode(TreeNode node) {
    ...
    if ( node.getColor()==Color.RED ) prod=(prod*node.getValue())%c;
  }

  @Override
  public void visitLeaf(TreeLeaf leaf) {

    if ( leaf.getColor()==Color.RED ) prod=(prod*leaf.getValue())%c;
  }
}
...
class FancyVisitor extends TreeVis {
  ...
  public void visitNode(TreeNode node) {
    ...
    if ( node.getDepth()%2==0 ) sum1+=node.getValue();
  }

  @Override
  public void visitLeaf(TreeLeaf leaf) {

    if ( leaf.getColor()==Color.GREEN ) sum2+=leaf.getValue();
  }
}

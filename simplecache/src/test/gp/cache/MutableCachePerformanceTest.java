...
public class MutableCachePerformanceTest {
	...
	public void setupCache() {
		...
		cache = new MutableEntityTagCache<>();
	}
	...
	private void doWritesAndReads(int times) {
		...
		System.out.println("MutableEntityTagCache (ms): " + duration);
		...
		start = System.currentTimeMillis();
		for (int i = 0; i < times; i++) {
			ehcache.put(new Element(i, Arrays.asList("value1u" + i, "value2u" + i)));
		}
		duration = System.currentTimeMillis() - start;
		System.out.println("Ehcache (ms): " + duration);
		...
		start = System.currentTimeMillis();
		for (int i = 0; i < times; i++) {
			cache.updateTags(i, Arrays.asList("value1u" + i, "value2u" + i));
		}
		duration = System.currentTimeMillis() - start;
		System.out.println("MutableEntityTagCache (ms): " + duration);
